    ...
    char *str, *del;
{
     ...
     * or, one more.
     */
	if (slen < len || strcmp(str + slen - len, del) != 0) {
		num++;
	}
    ...
    struct object *ob;
{
    ...
    ret = allocate_array (ob->prog->p.i.num_inherited + 1);

    for (inh = 0; inh < ob->prog->p.i.num_inherited; inh++) {
      ret->item[inh].type = T_STRING;
      ret->item[inh].subtype = STRING_SHARED;
      ret->item[inh].u.string = make_shared_string(
                                  ob->prog->p.i.inherit[inh].prog->name);
      }
    ret->item[inh].type = T_STRING;
    ret->item[inh].subtype = STRING_SHARED;
    ret->item[inh].u.string = make_shared_string(ob->prog->name);
...
char *str;
{
	...
	if ((sl > 2) && (str[sl-2] == '.') && (str[sl-1] == 'c')) {
		...
		*/
		p = (char *)MALLOC(sl - 1); /* (sl - 1) == minus ".c" plus "\0" */
		strncpy(p, str, sl - 2);
		p[sl - 2] = '\0';
		sl -= 2; /* removed the ".c" */
		str = p;
	...
	if (!(tmp_children
		= (struct object **) MALLOC(sizeof(struct object *) * (t_sz = 50))))
	{
	...
	for (i = 0, ob = obj_list; ob; ob = ob->next_all) {
			...
			&& !strncmp(str,ob->name,sl))
		{
			...
			if ((++i == t_sz) &&(!(tmp_children
				= (struct object **)REALLOC((void *)tmp_children,

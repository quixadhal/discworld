    ...
    char *name;
{
	...
	default:
	    ...
	    buf = (char *)
           DMALLOC(sizeof (long) + len + 1, 256, "socket_write: default");
	    if (buf == NULL)
	...
	case T_STRING:
	    ...
	    buf = (char *)DMALLOC(len + 1, 256, "socket_write: T_STRING");
	    if (buf == NULL)
		crash_MudOS("Out of memory");
    ...
    int fd;
{
	...
	case MUD:
		...
		lpc_socks[fd].r_buf = (char *)
           DMALLOC(lpc_socks[fd].r_len + 1, 256, "socket_read_select_handler");

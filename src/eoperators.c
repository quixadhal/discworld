  ...
  switch((sp-1)->type)
	...
	switch(sp->type)
	  ...
	  case T_STRING:
	    {
	      char *res;
          int len = strlen((sp-1)->u.string)+strlen(sp->u.string)+1;
	      res=XALLOC(len);
          eval_cost += (len >> 3);
	  ...
	  case T_NUMBER:
	    {
          ...
          len = strlen((sp-1)->u.string) + strlen(buff) + 1;
	      res = XALLOC(len);
          eval_cost += (len >> 3);
	  ...
	  case T_STRING:
	    {
          ...
          len = strlen(sp->u.string) + strlen(buff) + 1;
	      res = XALLOC(len);
          eval_cost += (len >> 3);
	      strcpy(res, buff);
...
int num_arg, instruction;
{
  ...
  case T_STRING:
      ...
      else if (sp->type == T_NUMBER)
	{
      ...
      int len;
	  sprintf(buff, "%d", sp->u.number);
      len = strlen(argp->u.string) + strlen(buff) + 1;
      eval_cost += (len >> 3);
	  new_str = XALLOC(len);
	  strcpy(new_str, argp->u.string);
	  strcat(new_str, buff);
...
#ifdef F_CALL_FUNCTION_BY_ADDRESS
INLINE void
...
int num_arg, instruction;
   ...
   *           char   index into function-list
   *           char   number of arguments
   */
{
  struct function *funp;
  int inh, fix, num_args;

#ifdef COUNT_CALLS
  num_call_down++;
#endif

  inh = EXTRACT_UCHAR(pc);
  pc++;
  fix = EXTRACT_UCHAR(pc);
  pc++;
  num_args = EXTRACT_UCHAR(pc);
  pc++;
    
  funp = &(current_prog->p.i.inherit[inh].prog->p.i.functions[fix]);
#ifdef DEBUG
  if (funp->flags & NAME_PROTOTYPE)
    fatal ("CALL_DOWN: Attempted call to prototype");
#endif
  /* Urgle. There should probably be a function for all this. |D| */
  /* See call-self for comments */
  push_control_stack (funp);
  variable_index_offset += 
       current_prog->p.i.inherit[inh].variable_index_offset;
  current_prog = current_prog->p.i.inherit[inh].prog;
...
int num_arg, instruction;
/* Get the function name, look for it in the tables of the current
 * object. If neither the current object nor any of the objects it
 * inherits defines the function, try calling the function in the
 * simul_efun file. If that also fails, generate an error.
 * (Perhaps push 0 instead of an error? I'd rather not, but
 *  it may be necessary for resets of inherited objects). */
{
  ...
  if (search_for_function (current_prog->p.i.strings[func_name_index],
                           current_object->prog)
      && (function_prog_found == current_prog ||
          !(function_type_mod_found & TYPE_MOD_PRIVATE)) ) {
    funp = &function_prog_found->p.i.functions[function_index_found];
    /* Save all important global stack machine registers */
#if DEBUG
    if (funp->flags & NAME_PROTOTYPE)
	fatal ("CALL_SELF: Attempted call to prototype");
#endif
    push_control_stack(funp);
      /* return pc is adjusted later */

    if (function_inherit_found == current_object->prog->p.i.num_inherited)
      variable_index_offset = 0;
    else
      variable_index_offset = 
	  current_object->prog->p.i.
             inherit[function_inherit_found].variable_index_offset;
    ...
    csp->extern_call = 0;
    return ;
  } else if (simul_efun_file_name &&
            /* Safety check */
             current_prog->name != simul_efun_file_name) {
     struct object *s_ob = find_object2 (simul_efun_file_name);
     if (s_ob != 0 &&
         apply_low (current_prog->p.i.strings[func_name_index], 
                    s_ob, num_args) )
  ...
  /* Not found! */
  { char buff[200];
    sprintf (buff, "Local function call %s not found", 
            current_prog->p.i.strings[func_name_index]);
    error (buff);
    }
  return ;
} /* f_call_self() */

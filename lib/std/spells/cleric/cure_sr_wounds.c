...
mixed cure_sr_wounds(string str, int skill, int cast);
 
string help() {
  return
"Cure serious wounds can be cast on a person or on yourself.  It gives you "+
"back a number of hit points.  "+
"It costs "+SP_COST+" spell points to cast, the amount of cureing done "+
...
int cast_cure_sr_wounds(string str) {
  int skill;
  mixed ret;
  ...
  ret = cure_sr_wounds(str, skill, 1);
  if (stringp(ret)) {
    notify_fail(ret);
    return 0;
    }
  write("You cast Cure Serious Wounds.\n");
  return 1;
  }
...
mixed cure_sr_wounds(string str, int skill, int cast) {
  ...
  if (sizeof(ob))
    ...
    ob = 0;
  this_player()->add_spell_effect(1, "spell", "Cure Serious Wounds",
    this_object(), "hb_cure_sr_wounds", ({ skill/DIVISOR,ob,cast }));
  return 1;
  }
 
int hb_cure_sr_wounds(object attacker, mixed *params) {
  int hp;
  ...
  tell_object(params[1], (string)previous_object()->query_cap_name() +
    " cures some of your serious wounds.\n");
  tell_object(previous_object(), "You cure some of " +
    (string)params[1]->query_cap_name() + "'s serious wounds.\n");
  hp = (int)params[1]->query_hp();
  if (hp == (int)params[1]->query_max_hp()) return 0;
  return ((int)params[1]->adjust_hp(params[0])-hp)/10 + 100;
  }

...
/* should check to see that we can give things to this person/thing... */
  for (;j<sizeof(per);j++) {
    ...
    if (sizeof(ret)) {
      ...
      if ((max = (int)per[j]->query_max_weight()))
      ...
      this_player()->adjust_time_left(-tot);
    ...
    for (j=0;j<sizeof(ret);j++)
      if (sizeof(ret[j])) {
        ...
        write("You "+({ "get", "get with a bit of difficulty",
         ...
         if (!this_object()->query_hidden())
          say(capitalize((string)this_object()->short())+" "+
...
/* We have been stopped!  How rude :) */
    ...
    for (i=0;i<sizeof(obs);i++) {
      ...
      if (tot - weight > 0)
        ...
        write("You interupt "+arg[2]+"ing "+arg[3]+" "+per[j]->short()+
              ", only "+arg[2]+"ing "+sh+".\n");
        say(this_player()+" interupts "+arg[2]+"ing "+arg[3]+" "+
...
/* Keep going... */
    switch (arg[2]) {
      case "give" :
      case "put" :
        give(0, arg[2], per, thing, j);
        break;
     case "get" :
        take(0, arg[2], per, thing, j);
        break;
    }
  }
}

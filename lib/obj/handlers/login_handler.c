...
void update_alignment(string name, int align) {
  ...
  if (sizeof(alignment[1]))
    str1 = alignment[1][sizeof(alignment[1])-2];
  if (sizeof(alignment[0]))
    str2 = alignment[0][sizeof(alignment[0])-2];
/* First remove useless frogs. */
  ...
  for (i=0;i < sizeof(alignment[1]) && (align>alignment[1][i]);i+=3);
  alignment[1] = alignment[1][0..i-1] + ({ align, name, time() }) +
                 alignment[1][i..1000];
  if (sizeof(alignment[1]) > (4*3)) alignment[1] = alignment[1][3..100];

/* First remove useless frogs. */
  for (i=0;i<sizeof(alignment[0]);i+=3)
    if (alignment[0][i+1] == name || alignment[0][i+2]+TIME_OUT > time()) {
      alignment[0] = alignment[0][0..i-1] + alignment[0][i+3..1000];
      i -= 3;
    }
  for (i=0;i < sizeof(alignment[0]) && (align<alignment[0][i]);i+=3);
  alignment[0] = alignment[0][0..i-1] + ({ align, name, time() }) +
                 alignment[0][i..1000];
  if (sizeof(alignment[0]) > (4*3)) alignment[0] = alignment[0][3..100];
...
string query_al_title() {
  ...
  if (member_array(name, alignment[0]) >= sizeof(alignment[0])-2)
    if ((int)previous_object()->query_gender() != 2)
      return "the lord of light";
    else
      return "the lady of light";
} /* query_al_title() */

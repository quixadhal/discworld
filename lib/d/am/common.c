#define DOM_NAME "the domain of Ankh-Morpork"
#define LORD "drgoon"
#define BB_NAME "am"
...
void setup() {
  set_light(100);
  set_short("Common room of "+DOM_NAME);
  set_long("Large relaxing chairs addorn the room.  The walls are covered "+
           ...
           DOM_NAME+" the large red letters on the wall were the give away.\n");
  ...
  add_item("motif", "The motifs on close inspection look like stylised "+
                     "signatures of all the members of "+DOM_NAME+
...
int sit(string str) {
  if (str != "in chair")
    return 0;
  write("You sit down in a likely looking chair.\nAfter a few moments you "+
        "realise that the chair is not happy about being sat on, in fact "+
        "seems to be telling you that after a hard days work it doesnt "+
        "deserve to be sat on.  You get up.... after the chair gives you "+
        "a little help.\n");
  say(this_player()->query_cap_name()+" sits down in a chair.\n");
  say(this_player()->query_cap_name()+" looks worried as the chair moves "+
        "a bit restlessly.\n");
  say(this_player()->query_cap_name()+" looks a bit miffed as the chair "+
        "throws him out and glares sullently at him.\n");
  return 1;
}

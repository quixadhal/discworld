...
int valid_read(string *path, string euid, string function) {
  int i;

  if (members[euid]) return 1;
  if (sizeof(path) < 3) return 1;
  switch (path[2]) {
    case "meetings" :
      return 0;
    default :
...
int valid_write(string *path, string euid, string function) {
  int i;
